...
public class ForgejoService(IConfiguration config) : IForgejoService
{
    private RestClient _client;
    
    public void Authenticate()
    {
        var provider = config.GetProvider("forgejo");
    ...
    public async Task<string> CreateRepository(Organization organization, ProjectInfo projectInfo)
    {
        ...
        {
            throw new NovugitException("Failed to create repository on Forgejo", "forgejo", e);
        }
    }
